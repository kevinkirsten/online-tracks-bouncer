...
var SoundTouchProcessor = class extends AudioWorkletProcessor {
	...
	_samples;
	_outputSamples;
	pendingInterpolationStrategy;
	pendingInterpolationStrategyParams;
	pendingStretchParameters;
	_underrunCount;
	_blockCount;
	/**
	...
	*/
	constructor(options) {
		super();
		let interpolationStrategy = options?.processorOptions?.interpolationStrategy;
		try {
			if (interpolationStrategy) resolveInterpolationStrategy(interpolationStrategy);
		} catch (err) {
			console.info("[SoundTouchProcessor] Unknown interpolation strategy id:", interpolationStrategy, "— falling back to lanczos.");
		...
		if (port !== void 0) port.onmessage = (event) => {
			...
			if (message.type === "set-stretch-parameters") this.pendingStretchParameters = message.params;
		};
	}
	applyPendingRuntimeUpdates() {
		if (this.pendingInterpolationStrategy !== null) {
			try {
				...
				console.info("[SoundTouchProcessor] Failed to switch interpolation strategy:", this.pendingInterpolationStrategy);
			}
			this.pendingInterpolationStrategy = null;
		}
		if (this.pendingInterpolationStrategyParams !== null) {
			try {
				this._pipe.setInterpolationStrategyParams(this.pendingInterpolationStrategyParams);
			} catch (err) {
				console.info("[SoundTouchProcessor] Failed to update interpolation strategy params.");
			}
			this.pendingInterpolationStrategyParams = null;
		}
		if (this.pendingStretchParameters !== null) {
			try {
				...
				console.info("[SoundTouchProcessor] Failed to update stretch parameters.");
			}
			this.pendingStretchParameters = null;
		}
	}
